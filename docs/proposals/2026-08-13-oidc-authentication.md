...
| # | Phase | Scope | Test evidence |
| --- | --- | --- | --- |
| 1 | This LEP | Design, phases, security model | n/a (prose) |
| 2 | Credential groundwork | `lore-credential`: shared decode helper, optional display claims, identity-entry update | `lore-credential` unit tests, `lore-revision` auth tests |
| 3 | Recipients and login flow types | `TokenRecipients` replaces `acceptable_root_domains`; `LoginFlow` parameter on `Authentication`; call sites | `lore-transport` unit tests |
| 4 | Server: provider discovery | `discovery.rs`; shared capped-fetch helpers in `jwk.rs` | `discovery.rs` unit tests against a local server |
| 5 | Server: OIDC verification mode | `JwtVerifierMode`, `OidcTokenClaims`, `OidcJwkService` symmetric-algorithm refusal | `jwt.rs`/`oidc_claims.rs`/`jwk.rs` unit tests |
| 6 | Server: configuration | `[server.auth.oidc]` settings and validation; config reference | `settings.rs` unit tests |
| 7 | Server: wiring | `build_jwt_verifier`, derived `auth_url`, advertised-environment copy | `server.rs` unit tests |
| 8 | Client: backend core | `oidc+https`/`oidc+http` parsing, discovery pinning, backend registration; all flows `NotSupported` | `oidc.rs` unit tests |
| 9 | Client: PKCE building blocks | Authorization URL, callback parsing, ID-token claims, credential mapping | `oidc.rs` unit tests |
| 10 | Client: browser login flow | Loopback listener, session table, start/poll | `oidc.rs` unit tests |
| 11 | Client: device login flow | RFC 8628 grant, poll schedule, `--no-browser` window | `oidc.rs` unit tests |
| 12 | Client: refresh grant | Refresh implementation and authorization passthrough | `oidc.rs` unit tests |
| 13 | Exchange integration | Single-flight refresh on the exchange path | `auth_exchange.rs` battery |
| 14 | Test infrastructure | Shared gRPC test bootstrap, compose service, feature gating | Pre-existing integration tests |
| 15 | Server integration tests | `PocketID` fixture; HTTP-served verification matrix | Live-provider tests |
| 16 | Client and gRPC integration tests | Login flows and gRPC verification against `PocketID` | Live-provider tests |
| 17 | End-to-end and docs | CLI smoke tests, CI job, how-to guide | Smoke suite in CI |
